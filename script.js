...
const stock = {
  ...
  removeItem(itemName, itemQuantity) {
    ...
    if (existItemIndex === -1) {
      alert(
        "Товара, который вы хотите удалить, нет на складе либо вы вводите некорректное кол-во удаляемых товар"
      );
      return;
    } else {
      if (itemQuantity > this.items[existItemIndex].quantity) {
        alert("Товар, который вы хотите удалить, недостаточно на складе");
        return;
...
statsBtn.onclick = () => {
  ...
  // Получаем массив цен из массива items - [5, 3, 25]
  const arrPrices = stock.items.map(e => e.price);
  console.log(arrPrices);
  const alternativeMaxPrice = Math.max(...arrPrices);
  const alternativeMinPrice = Math.min(...arrPrices);
  console.log(alternativeMaxPrice);
  console.log(alternativeMinPrice);
  ...
  // Мой вариант домашки:
  // const minPriceProduct = stock.items.reduce((acc, item) => acc < item.price ? acc : item.price, stock.items[0].price);
  // const averagePriceProduct = stock.items.reduce((acc, item) => acc + item.price, 0) / stock.items.length;
  // const maxPriceProduct = stock.items.reduce((acc, item) => acc > item.price ? acc : item.price, stock.items[0].price);
  // const totalProducts = stock.items.length;
  // const totalPriceProducts = stock.items.reduce((acc, item) => acc + item.price * item.quantity, 0);
  // const totalAmountProducts = stock.items.reduce((acc, item) => acc + item.quantity, 0);

  // Метод для чисел toFixed(x) - позволяет ограничить кол-во знаков после запятой до x знаков (от 0 до 20)
  //      В. Отобразить полученные статистические данные на странице
  statsList.innerHTML = `
  ...
  // li.textContent = `Минимальная стоимость продукта - ${minPriceProduct} $. Средняя цена товара - ${averagePriceProduct} $. Максимальная цена товара - ${maxPriceProduct} $. Общее кол-во позиций товаров - ${totalProducts}. Общая стоимость товаров - ${totalPriceProducts} $. Общее кол-во - ${totalAmountProducts}`;
  // productsList.appendChild(li);

  // li.style = "border-radius: 5px; padding: 5px 0 5px 5px; font-size: 30px";
};
...
function addProductToDom(item) {
  ...
  // HW Advanced (добавить кнопку plus (+) и minus (-) для изменения кол-ва товара)
  li.innerHTML = `
  <span>
...
// 1 вариант
  ...
  // 3 вариант (моя домашка)
  // class="btn btn-danger deleteBtn"
  // const deleteBtn = li.querySelector('.deleteBtn');

  // deleteBtn.onclick = () => {
    
  //   const index = stock.items.findIndex((e) => e.name === item.name);
  //   const existLi = document.getElementById(`product - ${item.name}`);
  ...
  //   if (existLi) {
  //     productsList.removeChild(existLi);
  //   }
  // }
